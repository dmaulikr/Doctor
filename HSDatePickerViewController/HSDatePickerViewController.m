...
//    [self setPickerView:self.pickerView rowInComponent:MinutePicker toIntagerValue:[components minute]  decrementing:NO animated:NO];
    ...
    NSDateComponents *components = [[NSCalendar currentCalendar] components:(NSCalendarUnitYear) fromDate:self.date];
    [self setPickerView:self.pickerView rowInComponent:YearPicker toIntagerValue:[components hour] decrementing:NO animated:NO];
//    [self setPickerView:self.pickerView rowInComponent:MinutePicker toIntagerValue:[components minute]  decrementing:NO animated:NO];
...
//    }
    if (firstComponentRowValue <= self.minRowIndex) {
        NSDateComponents *components = [[NSCalendar currentCalendar] components:(NSCalendarUnitYear) fromDate:self.minDate];
        [self setPickerView:self.pickerView rowInComponent:YearPicker toIntagerValue:[components hour] decrementing:NO animated:YES];
    }
...
//    }
    ...
    if (firstComponentRowValue >= self.maxRowIndex) {
        NSDateComponents *components = [[NSCalendar currentCalendar] components:(NSCalendarUnitYear) fromDate:self.maxDate];
        [self setPickerView:self.pickerView rowInComponent:YearPicker toIntagerValue:[components year] decrementing:YES animated:YES];
    }
...
//    return [[NSCalendar currentCalendar] dateBySettingHour:hour minute:minute second:0 ofDate:date options:0];
    ...
    NSInteger year = [[self pickerView:self.pickerView attributedTitleForRow:[self.pickerView selectedRowInComponent:YearPicker] forComponent:YearPicker].string integerValue];
    
    return [[NSCalendar currentCalendar] dateBySettingUnit:NSCalendarUnitYear value:year ofDate:date options:0];
